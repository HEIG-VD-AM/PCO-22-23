...
void emergency_stop()
{
    LocomotiveBehavior::emergencyStopped = true;
    locoA.arreter();
    locoB.arreter();
...
int cmain()
{
    ...
    std::shared_ptr<SharedSectionInterface> sectionPartagee = std::make_shared<SharedSection>();

    // Point départ train et gare
    const int POINT_DE_CONTACT_A = 33;
    const int POINT_DE_CONTACT_B = 29;

    // Nbre de tours devant réaliser chaque train avant d'inverser leur sens
    const int NB_TOURS_A = 1;
    const int NB_TOURS_B = 2;


    const std::vector<int> POINTS_CONTROLE_ACCEPTATION_A = {25, 14};
    const std::vector<int> POINTS_CONTROLE_ACCEPTATION_B = {22, 10};
    // Points de contrôle des aiguillages
    const std::vector<int> POINTS_CONTROLE_REQUETE_A = {24, 15};
    const std::vector<int> POINTS_CONTROLE_REQUETE_B = {21, 11};
    const std::vector<int> POINTS_CONTROLE_AIGUILLAGE_AB = {19, 13};

    // Etat que doit avoir chaque aiguillage pour le passage de son train
    const std::vector<std::pair<int, int>> ETAT_AIGUILLAGE_A = {{13, DEVIE}, {10, DEVIE}};
    const std::vector<std::pair<int, int>> ETAT_AIGUILLAGE_B = {{13, TOUT_DROIT}, {10, TOUT_DROIT}};
    ...
    std::make_unique<LocomotiveBehavior>(locoA, sectionPartagee,
                                         POINT_DE_CONTACT_A,
                                         NB_TOURS_A,
                                         ETAT_AIGUILLAGE_A,
                                         POINTS_CONTROLE_REQUETE_A,
                                         POINTS_CONTROLE_AIGUILLAGE_AB,
                                         gare,
                                         PRIO_A,
                                         POINTS_CONTROLE_ACCEPTATION_A);
    // Création du thread pour la loco 1
    std::unique_ptr<Launchable> locoBehaveB =
    std::make_unique<LocomotiveBehavior>(locoB, sectionPartagee,
                                         POINT_DE_CONTACT_B,
                                         NB_TOURS_B, ETAT_AIGUILLAGE_B,
                                         POINTS_CONTROLE_REQUETE_B,
                                         POINTS_CONTROLE_AIGUILLAGE_AB,
                                         gare,
                                         PRIO_B,
                                         POINTS_CONTROLE_ACCEPTATION_B);
